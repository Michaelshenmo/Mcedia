...
public final class MediaTextureImpl extends AbstractTexture implements MediaTexture {
    ...
    private void flushDirect(int width, int height, ByteBuffer directBuffer) {
        ...
        int writeTex = (writeIndex == 0) ? textureA : textureB;

        if (uploader == null) {
            uploader = new GlTextureUploader();
        }
        uploader.upload(writeTex, width, height, directBuffer, writeIndex);
    ...
    private void ensureTextures(int width, int height) {
        if (currentWidth == width && currentHeight == height && textureA != 0) {
            return;
        }
        ...
        for (int tex : new int[]{textureA, textureB}) {
